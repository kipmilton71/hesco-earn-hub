...
-- Add data validation constraints (using DO block to avoid IF NOT EXISTS issue)
DO $$
BEGIN
  -- Add email validation constraint if it doesn't exist
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint 
    WHERE conname = 'valid_email' AND conrelid = 'public.profiles'::regclass
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint 
    WHERE conname = 'valid_phone' AND conrelid = 'public.profiles'::regclass
  ) THEN
    ALTER TABLE public.profiles 
